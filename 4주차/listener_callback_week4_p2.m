function listener_callback_week4_p2(src,event)
...
%=============================================2-3)=============================================%
    ...
    if length(data_stack) == length(event.Data)
        for i= 2:length(event.Data)
            if  event.Data(i,2) <= 1.5 && event.Data(i-1,1)<=1.5 && 1.5 < event.Data(i,1) 
                k(i,1)= k(i-1,1) - 1; %시계방향
            elseif 1.5 < event.Data(i,2) && event.Data(i-1,1)<=1.5 && 1.5 < event.Data(i,1)
                k(i,1)= k(i-1,1) + 1; %시계방향
            else
        ...
        if event.Data(1,2) <= 1.5 && data_stack(length(data_stack)-length(event.Data),1) <=1.5 && 1.5 < event.Data(1,1)
            k(1,1) = -1;              %시계방향     마지막 data_stack 값과 event.Data의 첫 값을 비교
        elseif (1.5 < event.Data(1,2)) && (data_stack(length(data_stack)-length(event.Data),1) <=1.5) && (1.5 < event.Data(1,1))
            k(1,1) = 1;               %반시계방향    마지막 data_stack 값과 event.Data의 첫 값을 비교
        end
        for i= 2:length(event.Data)
            if event.Data(i,2) <= 1.5 && event.Data(i-1,1)<=1.5 && 1.5 < event.Data(i,1)
                k(i,1)= k(i-1,1) - 1; %시계방향
            elseif 1.5 < event.Data(i,2) && event.Data(i-1,1)<=1.5 && 1.5 < event.Data(i,1)
                k(i,1)= k(i-1,1) + 1; %반시계방향
            else
                k(i,1)= k(i-1,1);     %현위치
            end
        end
    ...
    xlabel('Time [sec]');
    ylabel('Angle in real time [degree]');
end